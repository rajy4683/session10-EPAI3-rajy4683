...
def test_poly_properties(input_edges = 5, circumradius = 2):
    ...
    def get_poly_values(input_edges, circumradius):
        ...
        return interior_angle, side_length, apothem, area, perimeter
    
    input_edges = 5
    circumradius = 2
    poly = ConvexPolygon(input_edges, circumradius)
    
    interior_angle, side_length, apothem, area, perimeter = get_poly_values(input_edges, circumradius)
    
    assert str(poly) == f'ConvexPolygon Object: Edges:{input_edges} Radius:{circumradius}', f'actual: {str(poly)}'
    assert poly.get_vertices == input_edges, (f'Returned vertex count: {poly.get_vertices},'f' Expected count: {input_edges}')
    assert poly.get_edges == input_edges, f'Returned Edge count: {p.get_edges}, Expected: {input_edges}'
    assert poly._circumradius == circumradius, f'actual: {poly._circumradius}, Expected: {input_edges}'
    assert poly.get_interior_angle == interior_angle, (f'actual: {poly.get_interior_angle}, Expected: {interior_angle}')
    assert check_numerical_similarity(poly.get_area, area) , (f'Actual Area: {poly.get_area}, Expected: {area}')
    assert check_numerical_similarity(poly.get_length_of_side, side_length) , (f'Actual Side length: {poly.get_length_of_side}, Expected: {side_length}')
    assert check_numerical_similarity(poly.get_apothem, apothem), (f'Actual Apothem: {poly.get_apothem},Expected: {apothem}')
    assert check_numerical_similarity(poly.get_perimeter, perimeter), (f'Actual Perimeter: {poly.get_perimeter}, Expected: {perimeter}')
